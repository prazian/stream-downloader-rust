...
const PLAYER_PRIORITY: &[&str] = &[
    "yandex", "vh", "rutube", "youtube", "ok", "zen", "vk",
];
...
pub fn resolve(page: &FetchedPage) -> Result<ResolvedSource> {
    ...
    let dups = json
        ...
        .filter(|m| !m.is_empty())
        .ok_or_else(|| {
            Error::InvalidUrl("video unavailable or geo-restricted".into())
        })?;

    let entry =
        pick_dup(dups, &film_id).ok_or_else(|| Error::NoStreamsFound)?;
    let href = entry
        .pointer("/host/href")
...
fn pick_dup<'a>(
    dups: &'a serde_json::Map<String, Value>,
    film_id: &str,
) -> Option<&'a Value> {
...
fn noframes_json(html: &str) -> Result<Value> {
    let start_tag = html.find("<noframes").ok_or_else(|| {
        Error::InvalidUrl("page missing Yandex embed data".into())
    })?;
...
mod tests {
    ...
    fn parses_film_id_from_path_and_query() {
        let url =
            Url::parse("https://yandex.ru/video/preview/18025183134392203214")
                .unwrap();
        assert_eq!(
            film_id(&url).as_deref(),
            Some("18025183134392203214")
        );
        let url =
            Url::parse("https://yandex.com/video/preview/?filmId=42").unwrap();
    ...
    fn resolves_upstream_from_fixture() {
        ...
        let page = FetchedPage {
            info: PageInfo {
                url: Url::parse(
                    "https://yandex.ru/video/preview/18025183134392203214",
                )
                .unwrap(),
                title: "preview".into(),
            },
